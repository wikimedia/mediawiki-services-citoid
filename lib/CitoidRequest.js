...
class CitoidRequest {
	...
	constructor( req, app ) {
		...
		this.search = this.request.params.search[ 0 ];
	...
	build() {
		...
		this.apiUserAgent = this.request.headers[ 'api-user-agent' ];
	...
	fillBody() {
		...
		// Used for success ratio SLO to (loosley) exclude bots from this metric
		if ( this.apiUserAgent && /^MediaWiki-JS/i.test( this.apiUserAgent ) ) {
			this.stats.apiuseragent.increment( 1, [ 'mediawikijs' ] );
		} else {
			this.stats.apiuseragent.increment( 1, [ 'other' ] );
		}
