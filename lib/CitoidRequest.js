...
class CitoidRequest {
	...
	constructor( req, app ) {
		...
		this.event = { start: new Date( req.startTime || Date.now() ).toISOString() };
	...
	fillBody() {
		...
		if ( this.response.body ) {
			return Promise.reject( 'Body already filled' );
		}

		if ( !this.exporter ) {
			return Promise.reject( 'No exporter registered with citoid service.' );
		}
		...
		if ( !this.response.citations ||
			...
			if ( !this.response.error && this.request.outgoingRequestError ) {
				this.response.error = this.request.outgoingRequestError;
			}
		...
		return Promise.allSettled( exportPromises ).then( () => {
			if ( success ) {
			...
			this.event.duration = ( end - Date.parse( this.event.start ) ) * 1e6;

			if ( this.response.responseCode === 500 ) {
				this.logger.log( 'warn/citoidRequest', { msg: '500 internal server error: ', event: this.event, cr: this } );
			} else {
				this.logger.log( 'info/citoidRequest', { msg: 'Full response complete, sending: ', event: this.event, cr: this } );
			}
			return this;
		} );
	}
