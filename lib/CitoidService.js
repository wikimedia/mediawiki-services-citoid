...
class CitoidService {
	...
	requestFromURL( citation, cr, reqDOI ) {
		...
		const reject = ( error ) => {
			cr.logger.log( 'trace/CitoidService', error );
		...
		return unshorten( url, cr, citation )
			.then( ( cit ) => {
				...
				// Uses Zotero
				const zotReq = () => {
					cr.logger.log( 'debug/zotero', `Redirect detected to ${ cit.resolvedUrl }` );
					return this.zoteroService.zoteroWebRequest( cr, cit )
						.then(
							// Success
							( innerCr ) => {
								cr.logger.log( 'trace/zotero', 'Successfully retrieved body from Zotero' );
								cr.logger.log( 'info/CitoidService', {
									msg: `requestFromURL succeeded for url ${ cit.resolvedUrl }`,
									outgoingReqResult: { uri: cit.resolvedUrl, status: 200 }
								} );
								return innerCr;
							},
							// Failure
							() => {
								cr.logger.log( 'warn/zotero', `No Zot response available for ${ cit.resolvedUrl }` );
								// In most cases we catch this before Zotero does
								if ( cr.error && cr.error.responseCode === 415 ) {
									cr.logger.log( 'trace/CitoidService', 'requestFromURL failed' );
									citation.error = cr.error;
									return cr;
								} else {
									return this.scraper.scrape( cit, cr );
								}
							} )
						.catch( ( error ) => {
							cr.logger.log( 'warn/zotero', error );
				...
				if ( zotero ) {
					...
					// Scrape HTML using native scraper
					return this.scraper.scrape( cit, cr );
				}
