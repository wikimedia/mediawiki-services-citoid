				...
				unshorten(requestedURL, function(detected, expandedURL) {
					if (detected) {
						...
						zoteroWebRequest(expandedURL, format, function(error, response, body){
							if (response && Array.isArray(response.body) && response.body[0] && !error && response.statusCode === 200){
...
CitoidService.prototype.requestFromDOI = function (doiOpts, callback){
	...
	request.head(resolveOpts, function (err, res) {
		if (!err && res && res.statusCode > 300 && res.statusCode < 400 &&
			...
			citoidService.requestFromURL(urlOpts, function(error, statusCode, body){
				// Ensure itemType in citation is journalArticle
				if (!body[0].itemType || (body[0].itemType && body[0].itemType !== 'journalArticle')){
...
CitoidService.prototype.distinguish = function(rawSearchInput, callback){
	...
	if (matchHTTP || matchWWW){
		...
		callback(matchHTTP ? encodeURI(matchHTTP[0]) : 'http://' + encodeURI(matchWWW[0]), this.requestFromURL.bind(this));
	} else if (matchDOI) {
		this.stats.increment('input.doi');
		callback(encodeURI(matchDOI[0]), this.requestFromDOI.bind(this));
	} else if (matchPMID) {
...
CitoidService.prototype.scrape = function(opts, callback){
	...
	var selectFormatFcn = this.zoteroService.selectFormatFcn.bind(this.zoteroService);
	...
	if (format === 'bibtex') {
		...
		scrape(opts, function(error, scrapeResCode, citation) {
			selectFormatFcn(format, function(convert){
				convert(opts.search, format, citation, function(citation){
