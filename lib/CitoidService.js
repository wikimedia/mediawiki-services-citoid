...
function CitoidService(app) {
    ...
    if (this.conf.wskey) {
        this.xml = new XMLReader(app, this.translator); // Translates from worldcat search API utilising services
    }
    ...
    if (xisbn || wskey) { // Enable isbn usage
        if (reISBN.test(search)) { // Original isbn match is too lenient; this gets actual isbn more accurately by cleaning out non-numerical characters
            ...
            if (match) {
                ...
                if (wskey) {
                    cr.getResponse = this.requestToWSKEY.bind(this);
                } else if (xisbn) {
                    cr.getResponse = this.requestToXISBN.bind(this);
                }
                this.stats.increment('input.' + cr.idType);
                return cr;
            }
 ...
 * @return {Object}          BBPromise object for modified CitoidRequest object from input
 */
CitoidService.prototype.requestToXISBN = function(cr) {
    ...
    return preq(requestOptions).then(
        function (res) {
            ...
            if (res && res.status === 200 && res.body.stat === 'ok') {
                ...
                if (res.body.list && res.body.list[0] && res.body.list[0].oclcnum &&
                    res.body.list[0].oclcnum[0] && typeof res.body.list[0].oclcnum[0] === 'string') {
 ...
 * @return {Object}          BBPromise object for modified CitoidRequest object from input
 */
CitoidService.prototype.requestToWSKEY = function(cr) {
    ...
    return preq(requestOptions).then(
        function (res) {
            if (res && res.status === 200) {
                return citoidService.scrapeXML(cr, res.body);
            } else {
                return reject(cr);
            }
        },
...
CitoidService.prototype.requestFromPM = function(cr){
    ...
    // Only create object if Zotero is enabled
    if (zotero) {
        zoteroWebRequest = self.zoteroService.zoteroWebRequest.bind(self.zoteroService);
    }
