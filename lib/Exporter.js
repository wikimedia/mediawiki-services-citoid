...
function fixISBN(citation){
    var match, i, reDash, reISBN,
        isbn = citation.ISBN;

    reDash = new RegExp('[\\-–]', 'g');
    reISBN = new RegExp('((978 ?)[0-9]{10}|[0-9]{9}[0-9xX])', 'g');

    if (isbn) {
        // Clean up ISBN so we can parse it easier
        isbn.trim();
        isbn = isbn.replace(reDash, '');
        ...
        if (match) {
            ...
            for (i in match){
                ...
                isbnMatch = isbnMatch.replace(' ', ''); // Remove any spaces (e.g. 978 0810935310)
...
function fixDate(citation){
    if (citation.date){
        ...
        var d = new Date(citation.date + (/[0-9]T[0-9]/.test(citation.date) ? '' : ' GMT+00'));
        if (isFinite(d)) {
