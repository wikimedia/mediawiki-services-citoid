...
class Exporter {
	...
	export( citation, request ) {
		if ( !citation.format ) {
			return Promise.reject( 'No format available to convert to' );
		}
		const convert = this.selectFormatFcn( citation.format );
		if ( !citation.content ) {
			return Promise.reject( 'No citation available to convert' );
		}
		return Promise.resolve( convert( citation, request ) );
	}
	 ...
	 * @param  {Object}    request   original request object
	 * @return {Promise}             Promise for modified Citation object from input
	 */
