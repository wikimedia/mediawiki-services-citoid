 ...
 */
function addIdentifiersToContent(citation) {
...
function addIDSToCitation(citation) {
    ...
    // Get additional IDs
    return addIdentifiersToContent(citation);
...
function fetchPubMedIDs(citation) {
    let gotData = false;
    defaultLogger.log('trace/citoidRequest', "Requesting additional IDs from PubMed service");
    return pubMedRequest((citation.doi || citation.pmcid || citation.pmid),
        userAgent, defaultLogger)
    .then((results) => {
        defaultLogger.log('trace/citoidRequest', "Request to PubMed service completed");
