    ...
    if (!citation.hasRequestedPubMed) {
        ...
        .then((results) => {
            ...
            if (!content.PMCID && results.records[0].pmcid) {
                gotData = true;
                content.PMCID = `PMC${results.records[0].pmcid}`;
            }
        ...
        if (p && p[0] && p[0].start) {
            ...
            if (isFinite(d)) {
                ...
                if (p[0].start.knownValues.year && p[0].start.knownValues.month
                    ...
                    `${p[0].start.knownValues.year}-${p[0].start.knownValues.month}`;
                }
            }
        }
    } catch (e) {
            ...
            if (cit.source && (cit.source.length > 0)) {
                ...
                if (!fc.url) {
                    if (fc.doi) {
                        ...
                        fc.url = `https://www.ncbi.nlm.nih.gov/pmc/articles/${fc.PMCID}`;
                    } else if (fc.PMID) {
