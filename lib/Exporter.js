...
partialISO.pattern = () => { // eslint-disable-next-line security/detect-unsafe-regex
	return /([0-9]{4})[-//]([0-9xX]{1,2})([-//][0xX]{1,2})?$/g;
};
...
// TODO: Add to end
const journalFormat = {};
journalFormat.pattern = () => {
	return /(\d{4})\s+([a-z]{3})\s*-\s*[a-z]{3}$/ig;
};
...
function addWaybackParams( citation, content ) {
	...
	if ( !content.archiveUrl && citation.archiveUrl ) {
...
class Exporter {
	...
	addIDSToCitation( citation, request ) {
		...
		// Prefer resolved url even if url is already in response
		if ( citation.resolvedUrl ) {
			content.url = citation.resolvedUrl;
		}
