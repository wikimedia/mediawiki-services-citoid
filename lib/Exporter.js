...
Exporter.prototype.selectFormatFcn = function (format) {
    ...
    var formatFcns = {
        ...
        'basefields':self.convertToBaseFields
        };
...
Exporter.prototype.convertToMWDeprecated = BBPromise.method(function(citation){
    // Don't directly modify cr.response.content, this should remain in zotero format only.
    var content = extend(true, {}, citation.content);
        ...
        if (content.creators) {
            ...
            for (var z in zotCreators) {
                ...
                if (zotCreators[z].firstName){
                    content[creatorFieldName + "-first"] = zotCreators[z].firstName;
                }
                if (zotCreators[z].firstName){
                    content[creatorFieldName + "-last"] = zotCreators[z].lastName;
    ...
    citation.formattedContent = content; // Add in process fotmattedContent to citation before adding IDs
...
var addIDSToCitation = BBPromise.method(function(citation){
    ...
    } else if (citation.idType && citation.idType !== 'url' && !content[citation.idType.toUpperCase()]) {
...
function fixDate(citation){
    if (citation.date){
        try {
            var d;
            // Strip copyright symbol if date is in format c2009 or ©2009; common in Worldcat
