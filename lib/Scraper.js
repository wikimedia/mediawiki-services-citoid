 ...
 *
 * @param  {Citation}    citationObj   Citation object
...
function citationFromCR( citationObj, cr ) {
	...
	if ( !citationObj.archiveUrl ) {
		content.url = citationObj.resolvedUrl || citationObj.url; // Use resolved url
	} else {
		content.url = citationObj.url; // Unless it's an archive.org url
	}
...
function getTitle( url, chtml ) {
	...
	return chtml( '*[itemprop~="headline"]' ).first().text() || url;
}
 ...
 *
 * @param  {Citation} citationObj the citation object
 * @param  {Object}   metadata    metadata from html-metadata's parseAll function
 * @param  {Object}   logger      logger instance
 * @return {Citation}             the citation object with IDs added
 */
		...
		try {
			if ( metadata.dublinCore && metadata.dublinCore.identifier ) {
				if ( typeof metadata.dublinCore.identifier === 'string' &&
					reDOI.test( metadata.dublinCore.identifier )
				) {
					...
					for ( let i = 0; i < metadata.dublinCore.identifier.length; i++ ) {
						if ( !citationObj.doi &&
							...
							reDOI.test( metadata.dublinCore.identifier[ i ] )
						) {
				...
				if ( typeof metadata.highwirePress.doi === 'string' &&
					reDOI.test( metadata.highwirePress.doi )
				) {
					...
					for ( let i = 0; i < metadata.highwirePress.doi.length; i++ ) {
						if ( !citationObj.doi && typeof metadata.highwirePress.doi[ i ] === 'string' &&
							reDOI.test( metadata.highwirePress.doi[ i ] )
						) {
				...
				if ( typeof metadata.bePress.doi === 'string' &&
					reDOI.test( metadata.bePress.doi )
				) {
					...
					for ( let i = 0; i < metadata.bePress.doi.length; i++ ) {
						if ( !citationObj.doi && typeof metadata.bePress.doi[ i ] === 'string' &&
							reDOI.test( metadata.bePress.doi[ i ] )
						) {
...
class Scraper {
	 ...
	 *
	 * @param  {Citation}  inputCitation  Citation object
	 * @param  {Object}    cr             CitationRquest object
	 * @param  {string?}   uri            Optional uri for when it differs i.e. wayback archive urls
	 * @return {Object}                   modified CitoidRequest object
	 */
	scrape( inputCitation, cr, uri ) {
		const citationObj = inputCitation;
		let chtml;
		const logger = cr.logger;
		const url = uri || citationObj.resolvedUrl;

		// Ensures scraper won't be run if unshorten isn't run first.
		if ( !url ) {
			const msg = 'No resolved URL found in citation object';
			logger.log( 'debug/scraper', msg );
			return citationFromCR( citationObj, cr ).then( this.build4xx );
		}

		// Check if archive.org url is a /primary/ url to enhance citations; seperate from fallback.
		let archiveMatch = false;
		if ( citationObj.resolvedUrl ) {
			archiveMatch = citationObj.resolvedUrl.match(
				// eslint-disable-next-line security/detect-unsafe-regex
				/^(?:https?:\/\/)?web\.archive\.org\/web\/\d{1,14}\/(.+)$/
			);
			if ( archiveMatch ) {
				citationObj.archiveUrl = url;
				citationObj.url = archiveMatch[ 1 ]; // TODO: This is getting overwritten
			}
		}
			...
			if ( !response || response.status !== 200 ) {
				...
				if ( chtml ) {
					...
					return citationPromise.then(
						( citation ) => this.parseHTML( citationObj, cr, chtml ).then(
							// Success handler for parseHTML
							() => {
								// If this is a success for an archived url, but not a primary one
								if ( citationObj.archiveUrl && !archiveMatch &&
										citationObj.error ) {
									citationObj.source.unshift( 'Wayback Machine' );
									logger.log( 'info/scraper', {
										msg: `Scraped archived url at ${ url }`,
										outgoingReqResult: {
											error: citationObj.error.error,
											uri: citationObj.url,
											archive: citationObj.archiveUrl
										}
									} );
								} else {
									logger.log( 'info/scraper', {
								...
								// Clear any prior errors i.e. if wayback succeeds
								citationObj.error = null;
	 ...
	 * @param  {Object} chtml       Cheerio object with html loaded
	 * @return {Promise}            promise for citation object
	 */
	parseHTML( citationObj, cr, chtml ) {
		...
		return parseAll( chtml )
			.then( ( metadata ) => {
				...
				return this.crossRef( citationObj, cr )
					.then( ( citoidRequest ) => addMetadata( metadata ),
						( e ) => {
	...
	crossRef( citationObj, cr ) {
		...
		if ( !doi ) {
			return Promise.reject( 'No DOI supplied' );
		}
	 ...
	 * @param  {Object} cr             CitoidRequest object
	 * @return {Promise}               Promise for CitoidRequest object
	 */
	build4xx( citationObj, cr ) {
		const url = citationObj.resolvedUrl || citationObj.url;
		...
		return this.crossRef( citationObj, cr ).then( ( citoidRequest ) => {
			...
			if ( !citationObj.error ) {
				if ( cr.request.outgoingRequestError ) {
					...
					citationObj.error = new CitoidError( null, `Unable to get any metadata from url ${ url } for unknown reasons`, 404 );
				}
			}

			cr.logger.log( 'warn/scraper', {
				msg: `Unable to get any metadata from url ${ url };
					returning 404 response.`,
				outgoingReqResult: { error: citationObj.error.error, uri: url }
			} );
