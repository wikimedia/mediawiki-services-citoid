...
function citationFromCR( citationObj, cr ) {
	...
	content.url = citationObj.resolvedUrl || citationObj.url; // Use resolved url, not original
...
class Scraper {
	...
	scrape( inputCitation, cr, uri ) {
		...
		const url = uri || inputCitation.resolvedUrl;
				...
				if ( chtml ) {
					...
					return citationPromise.then(
						( citation ) => this.parseHTML( citationObj, cr, chtml ).then(
							...
							() => {
								// If this is a success for an archived url, log explicitly
								if ( citationObj.archiveUrl && citationObj.error ) {
