...
/* Local Dependencies */
const crossRefRequest = require('./externalAPIs/crossRefRequest.js');
...
function matchIDs(citationObj, metadata, logger) {
    logger.log('debug/scraper', "Adding identifiers from metadata");

    // Try to get DOI from metadata before doing crossRef request
    if (!citationObj.doi) {
        const reDOI = new RegExp('\\b10\\.[0-9]{3,5}\\/(?:[^\\s]+)*');
                ...
                if (typeof metadata.dublinCore.identifier === 'string' &&
                    ...
                    for (let i = 0; i < metadata.dublinCore.identifier.length; i++) {
                        if (!citationObj.doi &&
                            ...
                            citationObj.doi = metadata.dublinCore.identifier[i].match(reDOI)[0];
                        }
                    }
                }
...
class Scraper {
    ...
    */
    scrape(citation, cr) {
        ...
        return unshorten(url, this.userAgent, cr.jar, this.conf, this.logger)
        .then((expandedURL) => {
            url = expandedURL;
            logger.log('debug/scraper', `Using native scraper on ${url}`);
            return preq({
                uri: url,
                followRedirect: false, // For security reasons, we use unshorten to follow redirects
    ...
    */
    parseHTML(citationObj, cr, chtml) {
        ...
        const addMetadata = (metadata) => {
            logger.log('debug/scraper', "Adding metadata using translators");
