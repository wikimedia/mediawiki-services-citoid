	...
	return preq({
		...
		if (!response || response.status !== 200) {
			...
			if (chtml){
				...
				return citationPromise.then(function(citation){
					return scraper.parseHTML(cr.doi, url, chtml, citation).then(
 ...
 * citation object
 *
 * @param  {String} doi             doi being scraped
 ...
 */
Scraper.prototype.parseHTML = function(doi, url, chtml, citation){

	function doSyncMethods(metadata, cit){
		// translator/openGraph.js properties
		if (metadata.openGraph){
			cit = openGraph(metadata.openGraph, cit);
		}

		// translator/general.js properties
		if (metadata.general){
			cit = translate(cit, metadata.general, gen.general);
		}

		// Fall back on direct scraping methods
		cit = fallback(cit, url, chtml);
	...
	.then(function(metadata){
		...
		return crossRef(doi, citation).then(function(cit){
 ...
 * @param  {Object} doi      DOI string
 * @return {Object}          BBPromise for citation object
 */
var crossRef = BBPromise.method(function(doi, citation){
	if (!doi){
		return BBPromise.reject('No DOI supplied');
	}
	return crossRefRequest(doi, defaultLogger).then(function(metadata){
...
function openGraph(metadata, citation){
	...
	var typeTranslator = og[citation.itemType];
	if (typeTranslator){
		citation = translate(citation, metadata, typeTranslator);
	}
...
var build520 = BBPromise.method(function(cr){
	...
	return crossRef(cr.doi, citation).then(function(cit){
...
var citationFromCR = BBPromise.method(function(cr){
	...
	if (cr.doi && (cr.itemType === 'journalArticle' || cr.itemType === 'conferencePaper')){
		citation.DOI = cr.doi;
	}
