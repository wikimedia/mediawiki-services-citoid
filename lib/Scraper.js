...
var Scraper = function(app, translator){
                ...
                return citationPromise.then(function(citation){
                    return scraper.parseHTML(cr, chtml).then(
                        ...
                        function(citation){
                            ...
                            cr.response.source.push('citoid'); // Credit citoid with a successful scraping :D
...
Scraper.prototype.parseHTML = function(cr, chtml){
    ...
    var addMetadata = BBPromise.method(function(metadata, cit){
        ...
        cit = translate(cit, metadata.highwirePress, bp[cit.itemType]);

        // Use bePress.js translator for bepress metadata
        cit = translate(cit, metadata.bePress, bp[cit.itemType]);
    ...
    .then(function(metadata){
        ...
        if (!cr.doi){
            ...
            if (metadata.dublinCore && metadata.dublinCore.identifier && metadata.dublinCore.identifier.match(reDOI)){
                ...
                cr.doi = metadata.highwirePress.doi.match(reDOI)[0];
            }
...
function addItemType(metadata, citation){
    ...
    if (!citation.itemType){ // Don't overwrite itemtype
        ...
        else if (metadata.dublinCore && metadata.openGraph['type'] && dc.types[metadata.dublinCore['type']]){ // if there is a type in the results and that type is defined in dublinCore.js
...
var build520 = BBPromise.method(function(cr){
    ...
    return crossRef(cr).then(function(citoidRequest){
        citoidRequest.response.citation[0] = fallback520(citoidRequest.response.citation[0], 200);
        defaultLogger.log('debug/scraper', "Sucessfully got metadata from doi " + cr.doi);
        return citoidRequest;
    // Rejection
    }, function(){
        defaultLogger.log('info/scraper', "Unable to get any metadata from doi " + cr.doi + "; returning 520 response.");
