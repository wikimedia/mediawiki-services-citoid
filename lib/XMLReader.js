...
/**
 * Translate Worldcat Search API XML
 */
...
const dc = require('./translators/wcDublinCore.js');
...
function getItemType(record) {
    ...
    if (record.type && Array.isArray(record.type) && record.type.length > 0) {
        ...
        for (i = 0; i < record.type.length; i++) {
          if (types[record.type[i]]) {
            itemType = types[record.type[i]];
            }
        }
    }
    ...
    translate(citationObj, cr, xml, wskeyFormat, creatorOverwrite) {
        ...
        return xml2js.parseStringAsync(xml).then((result) => {
            if (wskeyFormat === 'dc') {
                ...
                return this.worldCatService.singleRecordRequest(cr.request, con.oclc, 'oclc', 'dc')
                .then((body) => {
