...
const HTTPError = require( './utils/util.js' ).HTTPError;
...
class ZoteroService {
	 ...
	 * @return {Object}                 Promise for a response
	 */
	zoteroExportRequest( citation, format ) {
		...
		return Promise.resolve( fetch( url.toString(), {
			...
			// For bibtex format, response is plain text; for other formats, it's JSON
			const parseResponse = format === 'bibtex' ? res.text() : res.json();

			return parseResponse.then( ( body ) => {
				if ( !res.ok ) {
					// Mimic preq HTTPError structure
					throw new HTTPError( {
						status: res.status,
						body: body,
	...
	zoteroSearchRequest( id, cr, citation ) {
		...
		if ( typeof id !== 'string' ) {
			return Promise.reject( 'Invalid type; id must be a string' );
		}
		const options = {
			uri: this.searchURL,
			method: 'post',
		...
		return cr.request.issueRequest( options ).then( ( response ) => {
			...
			if ( response && response.status === 200 ) {
				...
				if ( response.body && Array.isArray( response.body ) && response.body[ 0 ] ) {
					...
					if ( Array.isArray( response.body[ 0 ] ) ) {
						if ( response.body[ 0 ][ 0 ] ) {
							...
							return Promise.reject( 'No citation in body' );
						}
					}
				...
				return Promise.reject( 'Non 200 response from Zotero' );
			}
		}, ( response ) => {
	...
	zoteroWebRequest( cr, citation ) {
		...
		if ( !requestedURL ) {
			return Promise.reject( 'No url in Citation object' );
		}
