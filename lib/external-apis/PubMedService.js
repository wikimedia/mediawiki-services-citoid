...
class PubMedService {
	 ...
	 * @return {Promise}             Promise for converted ID
	 */
	convert( identifier, request ) {
		if ( !identifier ) {
			return Promise.reject( 'No identifier in request' );
		}
		const escapedId = encodeURIComponent( identifier );
		// TODO: Make url configurable
		const url = 'https://pmc.ncbi.nlm.nih.gov/tools/idconv/api/v1/articles/';
