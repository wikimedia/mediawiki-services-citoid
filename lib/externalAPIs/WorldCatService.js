...
class WorldCatService {
    ...
    */
    ...
    openSearchRequest(search) {
        this.logger.log('debug/other', 'Making request to WorldCat Open Search service');
        if (!this.wskey) {
            return BBPromise.reject('No WSKEY in config');
        }

        // Format the XML citation will come back in. Can be DC, MarcXML,
        // or WorldCat's native openSearch format
        // XXX: wsKeyFormat is never used - intentional?
        // const wskeyFormat = 'openSearch';
