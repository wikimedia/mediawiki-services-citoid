 ...
 */
const hostIsAllowed = async ( url, conf, logger, requiresHost ) => {
	...
	if ( net.isIP( hostname ) !== 0 ) {
		...
		return dns.lookup( hostname )
			...
			.then( ( params ) => {
				...
				logger.log( 'trace/hostIsAllowed', `resolve4 ran: ${ addresses }` );
			...
			.then( ( params ) => {
				Array.prototype.push.apply( addresses, params );
				logger.log( 'trace/hostIsAllowed', `resolve6 ran: ${ addresses }` );
			} )
			.catch( ( err ) => {
