	...
	var pubMedRequest = function (identifier, logger, callback){
		...
		request(url, function(error, response, body){
			...
			if (error) {
				...
				logger.log('warn/pubmed', "Unexpected HTTP status code: " + response.statusCode);
				...
				try {
					...
					logger.log('debug/pubmed', "Original response: " + body);
					logger.log('warn/pubmed', "JSON parse error: " + error);
					callback("JSON parse error: " + error, null);
				}

				if (jsonObj){
					if (jsonObj.status !== 'ok'){
						logger.log('warn/pubmed', "Unexpected status from PubMed API: " + jsonObj.status);
						callback("Unexpected status from PubMed API: " + jsonObj.status, null);
					} else if (jsonObj.records.length === 0){
						logger.log('warn/pubmed', "No records from PubMed API");
