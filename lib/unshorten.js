	...
	const followRedirect = ( redirLocation, prevUrl ) => {
		...
		if ( !/^https?:\/\//i.test( redirLocation ) ) {
			try {
				...
				redirLocation = new URL( redirLocation, oldURL.href );
				redirLocation = redirLocation.href;
				request.logger.log( 'debug/unshorten', `Assembled relative redirect: ${ redirLocation }` );
			} catch ( e ) {
				request.logger.log( 'debug/unshorten', `Unable to assemble relative redirect: ${ e }` );
				request.outgoingRequestError = e; // Logs at warn level later if request fails
				throw e;
			}
		}
