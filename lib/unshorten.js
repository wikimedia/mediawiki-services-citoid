...
function unshorten(url, userAgent, jar, conf, logger) {
    logger.log('debug/unshorten', `Unshortening: ${url}`);
    const opts = {
    ...
    const followRedirect = (redirLocation, prevUrl) => {
        ...
        .then((allowedUrl) => {
            ...
            if (!parsedUrl.hostname) {
                ...
                logger.log('debug/unshorten', `Assembled relative redirect: ${allowedUrl}`);
            }
    ...
    .then((allowedUrl) => {
        ...
        .then(detectRedirect)
        .catch((error) => {
            logger.log('debug/unshorten', `Caught at recursion: ${error}`);
            throw error;
        });
    })
    .catch((error) => {
        logger.log('debug/unshorten', `Caught at top level${error}`);
        throw error;
    });
}
