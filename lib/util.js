...
function crForLog( cReq ) {
	...
	if ( cReq.response ) {
		...
		if ( cReq.response.error && cReq.response.error.error ) {
			...
			if ( ret.error.body && ret.error.body.internalURI ) {
				ret.uri = uriForLog( ret.error.body.internalURI );
			}
		} else {
			// Add url from CitoidRequest if no error
			// Might differ from actual resolved url
			ret.uri = uriForLog( cReq.url );
		}
	}
		...
		routerLayer.route.stack.forEach( ( layer ) => {
			...
			const metric = app.metrics.makeMetric( {
				...
				prometheus: {
					...
					buckets: [ 0.01, 0.05, 0.1, 0.3, 1, 60 ]
				},
				labels: {
