 ...
 * @param {boolean} nostack pass true if we don't want to log the stack even for 500s
 * @return {!Object} the serialized version of the error
 */
function errForLog( err, nostack ) {

	const ret = bunyan.stdSerializers.err( err );
	const stack = !nostack; // make truthy
	...
	if ( !stack || Number.parseInt( ret.status, 10 ) !== 500 ) {
		ret.stack = undefined;
	}
...
function outgoingReqResult( res ) {
	...
	if ( res.error && res.error instanceof Error ) {
		ret.error = errForLog( res.error, true );
		ret.status = ret.error.status;
		if ( ret.error.uri ) {
			ret.hostname = ret.error.uri.hostname;
			ret.uri = ret.error.uri.uri;
		}
	}
	if ( res.uri ) {
